...
class Slicer :
    ...
    def slice(self, pieces:List[Piece], offset:Position):
        ...
        for piece in pieces:
            ...
            z_offset = Position(0,0,-2,0) #TODO dealer ak ca
            pick_pos = Position(0, 0, 0, 0) #TODO dealer ak la position du PICK
            pick_offset = Position(0,-1,0,0) #TODO dealer pick ofset
            commands[f"component{piece_id}"] = {
                1 : {
                    "command" : Command.MOVE.value,
                    "speed" : self.speed,
                    "position" : (pick_pos + offset + storage_offset[piece]).toJSON()
                }, 
                2 : {
                    "command" : Command.PICK.value, 
                    "speed" : Z_SPEED, 
                    "position" : (pick_pos + pick_offset + offset).toJSON()
                }, 
                3 : {
                    "command" : Command.MOVE.value,
                    "speed" : self.speed,
            ...
            storage_offset[piece] = storage_offset[piece] + pick_offset
            piece_id += 1
